...
<div class="container">
    ...
    <div class="card">
        <form action="{{ route('leave_approving_officer.update', $leaveType) }}" method="post">
        @method('PUT')
        @csrf
            <div class="card-body">
                <div class="row">
        ...
        <div class="card-body">
            ...
            <table class="table table-striped table-bordered table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{$approving_officer->typeType}}</td>
                        <td>{{$approving_officer->userName}}</td>
                        <td>
                            
                            <form onsubmit="return confirm('Do you really want to delete it?');" action="{{ route('leave_approving_officer.delete', $approving_officer->id) }}" method="post">
                                @method('DELETE')
                                @csrf
            ...
            <div class="row justify-content-center">
                <h3>There are no approving officer for {{$leaveType->name}}</h3>
            </div>
            @endif
            </div>
        </div>
